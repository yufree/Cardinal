	...
	mass.range, resolution, units, BPPARAM)
{
	...
	if(mobility.ibdtype[1]=='') mobility.ibdtype <- rep("32-bit float",length(mobility.ibdtype))
	# read binary data
	if ( ibdtype == "continuous" ) {
		...
		if ( attach.only ) {
			...
			spectra <- sparse_mat(index=mapply(function(x,y) x*100000+y, mobility,mz), data=intensity,
